class User < ApplicationRecord
  ...
  has_many :active_relationships, class_name:   "Relationship",
                                  ...
                                  dependent:    :destroy#Realationshipクラスを明示的に示している,#foreign=外部キー
 ...
 # 永続セッションのためにユーザーをデータベースに記憶する
  ...
  def feed
    ...
    Micropost.where("user_id IN (#{following_ids})
                     OR user_id = :user_id", user_id: id)#現在ログインしているユーザーのマイクロポストをすべて取得
  end
  ...
  def unfollow(other_user)
    active_relationships.find_by(followed_id: other_user.id).destroy #削除する対象を見つけ削除する
  end
